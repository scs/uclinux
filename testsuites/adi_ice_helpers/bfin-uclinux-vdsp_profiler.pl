...
#
# TODO: Still need to load in module.maps somehow, please let me
...
while(<M>) {
	...
	if ( ! $symtab{sprintf("%x",(hex $tmp1[1]) -1)} ) {
		$symtab{sprintf("%x",(hex $tmp1[1]) -1)} = "Unknown Mapping";
	}
