...
int test_decoder(int mode, FILE *fbit, FILE *fout, float *av_mips, int multi, int thread_id)
{
  ...
  while (len != -1) {
	...
	fprintf(stderr, "diff :%d %hx, %hx\n", 
		frame_num*82 + i, inst_pcm_buf[i], result_buf[i]);
	return -1;
      }
    }
    frame_num++;		
