...
void rtp_session_flush_sockets(RtpSession *session){
	...
	if (rtp_session_using_transport(session, rtp))
	  {
	    while (session->rtp.tr->t_recvfrom(session->rtp.tr,trash,sizeof(trash),0,(struct sockaddr *)&from,&fromlen)>0){};

	    if (session->rtcp.tr)
	      while (session->rtcp.tr->t_recvfrom(session->rtcp.tr,trash,sizeof(trash),0,(struct sockaddr *)&from,&fromlen)>0){};
...
rtp_session_rtp_send (RtpSession * session, mblk_t * m)
{
	...
	if (rtp_session_using_transport(session, rtp))
	  error = (session->rtp.tr->t_sendto) (session->rtp.tr, m->b_rptr, (int) (m->b_wptr - m->b_rptr),
			 0,destaddr,destlen);
	else
...
rtp_session_rtcp_send (RtpSession * session, mblk_t * m)
{
	...
	if ((sockfd>=0 || rtp_session_using_transport(session, rtcp)) && (session->rtcp.rem_addrlen>0 ||using_connected_socket) ){
		...
		if (rtp_session_using_transport(session, rtcp))
		  error = (session->rtcp.tr->t_sendto) (session->rtcp.tr, m->b_rptr,
...
rtp_session_rtp_recv (RtpSession * session, uint32_t user_ts)
{
	...
	while (1)
	{
		...
		if (sock_connected){
		  ...
		  error = (session->rtp.tr->t_recvfrom)(session->rtp.tr, mp->b_wptr,
				  bufsz, 0,
				  (struct sockaddr *) &remaddr,
				  &addrlen);
