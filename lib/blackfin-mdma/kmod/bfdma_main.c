...
static ssize_t bfmdma_read(struct file *filp, char *buf, size_t count,
			   loff_t * f_pos)
{
	return 0;
}

static ssize_t bfmdma_write(struct file *filp, const char *buf, size_t count,
			    loff_t * f_pos)
{
	return 0;
}
...
static void bfmdma_pdesc(unsigned p)
{
	...
	for (i = 0; i < 2; i++) {
		stop = (x[2] & 0xff00) == 0;
		printk
		    ("%p: ARY SAH/L: %04x%04x CFG: %04x X(%d,+%d) Y(%d,+%d)\n",
...
static int __init bfmdmamod_init(void)
{
	...
	printk(KERN_INFO "bfmdma module init ex_dmach1: %p\n", ex_dmach1);
...
static void __exit bfmdmamod_exit(void)
{
	...
	bfmdma_proc_exit();
	unregister_chrdev(bfmdma_major, BFMDMA_DEV);
...
MODULE_DESCRIPTION("Blackfin fast userspace memory DMA driver");
MODULE_AUTHOR("Marc Hoffman");
MODULE_LICENSE("GPL");
