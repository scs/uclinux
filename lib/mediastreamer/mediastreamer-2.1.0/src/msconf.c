...
static void channel_init(ConfState *s, Channel *chan){
	ms_bufferizer_init(&chan->buff);
#ifndef DISABLE_SPEEX
	chan->speex_pp = speex_preprocess_state_init(CONF_GRAN/2, s->samplerate);
...
static void conf_sum(ConfState *s){
	...
	for (i=0;i<CONF_MAX_PINS;++i){
		...
		if (i>0 && ms_bufferizer_get_avail(&chan->buff)>=CONF_GRAN*6)
		{
			while (ms_bufferizer_get_avail(&chan->buff)>=CONF_GRAN*2)
			{
				...
				if (chan->speex_pp!=NULL && s->enable_vad==TRUE)
				{
					...
					ms_message("No voice detected: discarding some sample to sync incoming RTP with soundcard. (vad=%i - index=%i - bufsize=%i)",
						vad, i, ms_bufferizer_get_avail(&chan->buff));
				}
#endif
				chan->stat_discarded++;
			}
		...
		else if (ms_bufferizer_get_avail(&chan->buff)>=CONF_GRAN)
		{
			ms_bufferizer_read(&chan->buff,(uint8_t*)chan->input,CONF_GRAN);
#ifndef DISABLE_SPEEX
			if (chan->speex_pp!=NULL && s->enable_vad==TRUE)
	...
	if (s->mix_mode == FALSE)
	{
		...
		for (i=1;i<CONF_MAX_PINS;i=i+2){
			if (f->inputs[i]!=NULL){
				...
				if (chan->is_used==TRUE)
				{
					while (ms_bufferizer_read(&chan->buff,(uint8_t*)chan->input,CONF_GRAN)==CONF_GRAN)
					{
						if (f->outputs[0]!=NULL)
						{
							/* send in pin0 */
							mblk_t *m=allocb(CONF_GRAN,0);
							memcpy(m->b_wptr, chan->input, CONF_GRAN);
							m->b_wptr+=CONF_GRAN;
				...
				{
					while (ms_bufferizer_read(&chan0->buff,(uint8_t*)chan0->input,CONF_GRAN)==CONF_GRAN)
					{
						if (f->outputs[i]!=NULL)
						{
							/* send in pinI */
							mblk_t *m=allocb(CONF_GRAN,0);
							memcpy(m->b_wptr, chan0->input, CONF_GRAN);
							m->b_wptr+=CONF_GRAN;
							ms_queue_put(f->outputs[i],m);
						}
					}
