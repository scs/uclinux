 ...
 ******************************************************************************
  ...
  Registers Used : R0 to R7, P0 to P2, P5, M0 to M2, I0 to I3, B0 to B2
  ...
  Cycle count   :   1724 cycles (FFT length - 64)
...
ST_MIDDLE:R0 = P2;                              // Offset computation for fetching the... 
	R0 = R0 << 2;                           // butterfly inputs of every group...
	M0 = B2;
	I3 = 0;                                 // Initialize Butterfly count for each group
	R0 = R0.L*R3.L (IS) || R2 = [FP - 16];  // Offset =4*Group*Group size offset
	R2 = R0 + R2 (NS) || R0 = [FP - 8];     // Add Start address of each group and the Offset address
	I0 = R2;
