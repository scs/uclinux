 ...
 ******************************************************************************
  File Name    : cfftrad4.asm
  Module Name  : one dimensional radix 4 forward FFT for complex data.
  Label name   :  __cfftrad4_fr16
  
  Description  : The assembly function implements the radix4 FFT DIF algorithm.
		 ...
		 want to compute FFT. For FFT lengths less than 3*MAX_N/4, the same 
		 twiddle factor array can be used by choosing the stride factor appropriately, 
		 i.e., for an N-point FFT where N < MAX_N, the stride factor is MAX_N/N. 
		 For FFT length equal to MAX_N the stride factor is unity. The input and the 
		 output arrays are in normal order. 

		 Input data is scaled by 4 before each stage of radix 4 fft including 
		 the first stage to avoid overflow (Static scaling)
  ...
  Cycle count   :   1690 cycles (FFT length - 64)
...
ST_MIDDLE:R4 = P2;                              // Offset computation for fetching the... 
	R4 = R4 << 2 || R0 = [FP - 8];                  // butterfly inputs of every group...
	I3 = 0;                                 // Initialize Butterfly count for each group
	R4 = R4.L*R3.L (IS) || R2 = [FP - 16];  // Offset =4*Group*Group size offset
	R4 = R4 + R2;                           // Add Start address of each group and the Offset address
	I0 = R4;
	I1 = R4;                                // Effective address for fetching the butterfly inputs
	I2 = B1;                                // Save twiddle factor address
	I0 -= 4;
	I0 += M3;
	R4 = [I1 ++ M1];                        // Fetch the input of each butterfly in the group   

	LSETUP (ST_BFLY, END_BFLY) LC1 = P0;    // Do butterfly loop
ST_BFLY:    R1 = I3;                            // Twiddle factor array offset computation
...
LAST_STAGE: P1 = [FP-4] || NOP;                 // Initialize loop counter
