 ...
 ******************************************************************************
  File name      :  maxloc.asm 
 
  Description    : This program finds the location of maximum number 
 ...
 __vecmaxloc to __vecmaxloc_fr16
...
__vecmaxloc_fr16:
	    ...
	    LSETUP(ST_VMAXLOC,ST_VMAXLOC) LC0 = P1>>1;// LOOP FOR N/2 TIMES
ST_VMAXLOC: (R1,R0) = SEARCH R2(GT) || R2 = [P0++];   // SEARCH FOR FIRST MAXIMUM, FETCH NEXT TWO ELEMENTS

	    CC = BITTST(R7,0);                        // CHECK WHETHER EVEN
	    IF !CC JUMP EVEN_SMPL;                    // IF EVEN JUMP

	    R2.H = 0X8000;                            // IF ODD, R2.H = MIN. VALUE
	    (R1,R0) = SEARCH R2(GT);                  // IF ODD CHECK WITH LAST ELEMENT
