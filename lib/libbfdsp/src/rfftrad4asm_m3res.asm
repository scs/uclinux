 ...
 ******************************************************************************
  ...
  Cycle count   :   1711 cycles (FFT length - 64)

  Code size     :   780 bytes

  Code Modified on 11.01.2002 for removing the usage of M3 register
...
//*************************************** FIRST STAGE*****************************************************      
    ...
    R0 = [FP - 12];
    R2 = R3 << 2 || [FP - 8] = R0;
    R1 = R3 >> 2 || [FP - 20] = R2;         // Compute Group size = (State >> 2)
    M0 = R2;
    P0 = R1;                                // Initialize butterfly loop counter
    R4 = [FP - 16];                         // Offset =4*Group*Group size offset
    I0 = R4;
    I1 = R4;                                // Effective address for fetching the butterfly inputs
...
ST_BFLY1:   R4 = [I1 ++ M1];                // Fetch the input of each butterfly in the group   
	    ...
	    R0 = R0 +|+ R6 || R7 = [I1 ++ M1];  
	    R2 = R0 +|+ R7 || I1 -= M0 || R0 = [I0++];// Butterfly Offset
	    R2 = R2 >>> 2 (V) || I0 -= M0 || R0 = [I2 ++ M2];
	    ...
	    R4 = R6 -|+ R7 || R1 = [I2];
	    R4 = R4 >>> 2 (V);
	    R6.H = R5.H*R1.L, R6.L = R5.L*R1.L || [I0++M1] = R4 || R2 = [FP - 20];  // Multiply twiddle factor and the Butterfly o/p
	    R7.H = R5.L*R1.H, R7.L = R5.H*R1.H || I2 -= M0;
	    R0 = R6 -|+ R7 || I1 += 4;
	    R0 = R0 >>> 2 (V) || I3 += 4;
...
ST_MIDDLE:R0 = P2;                              // Offset computation for fetching the... 
	...
	R4 = R0 + R1 (NS) || R0 = [FP - 8];     // Add Start address of each group and the Offset address
	I0 = R4;
	I1 = R4;                                // Effective address for fetching the butterfly inputs
	I2 = B1;                                // Save twiddle factor address
	I0 -= 4;
...
ST_BFLY:    R4 = [I1 ++ M1];                    // Fetch the input of each butterfly in the group   
	    ...
	    R2 = R2 >>> 2 (V) || I0 -= M0 || R0 = [I2 ++ M2];   // I2 -> Dummy fetch
	    R0 = R1 +|+ R6 || [I0++M1] = R2;    // Compute the third o/p of the Butterfly           
	    R0 = R0 -|- R7 || R2 = [I2]; 
	    R4 = R4 -|- R6;                     // Compute the second and fourth o/ps of the Butterfly
	    R1.H = R0.H*R2.L, R1.L = R0.L*R2.L || I2 -= M2;// Multiply twiddle factor and the Butterfly o/p
