 ...
 ******************************************************************************
  ...
  Cycle count   :   1819 cycles (IFFT length - 64)
...
ST_MIDDLE:R0 = P2;                              // Offset computation for fetching the... 
	...
	R0 = R0 + R2;                           // Add Start address of each group and the Offset address
	I0 = R0;
...
ST_BFLY:
	    ...
	    R1 = R6 -|+ R7 || I1 += 4;                      
	    R1 = R1 >>> 2 (V) || I3 += 4;
END_BFLY:   [I0 ++ M1] = R1 || R4 = [I1 ++ M1]; // Store the fourth o/p of the Butterfly
...
LAST_STAGE: P1 = [FP-4];                    // Initialize loop counter
	    R0 = [FP - 16];                 // Save Temporary buffer address
	    I1 = R0;
	    M1 = 12;
