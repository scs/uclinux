 ...
 ****************************************************************************************
  File Name    : i2fftnasm.asm
  Module Name  : one dimensional radix 2 Inverse FFT for complex data.
  Label name   :  __ifftN_fr16
  Description  : This file contains the code for the implementation of IFFT.
		 The IFFT is implemented using the FFT computation. The input real and 
		 imginary parts are swapped with each other. Then the FFT computation is
		 done. In last again the real and imaginary part of output are swapped.
		 The algorithm used is Decimation in Time. 
		 For the optimization point of view, the whole computation of butterfly 
		 signal flow has beeen divided in three parts. The first part, the middle 
		 part and the last part. In the first part, the Stage 1 and Stage 2 of the
		 butterfly structure are implemented. In the 2nd part the general butterfly
		 computation is done, which corresponds to the middle stages of butterfly.
		 In the last part the last stage of the butterfly structure is implemented, 
...
Performance:
	     The Code Size = 454 Bytes.
	     The Cycle count for N = 32   =  534 cycles.
	     The cycle count for N = 256  = 5280 cycles.
****************************************************************************************/
...
*/
	...
	R5 = [I1];
	R6.H = (A1 += R3.H *R6.L), R6.L = (A0 -= R3.H * R6.H) || R3 = [I3++M3];
	[I2++] = R4;
	...
	I1 += M0;     // Before this there was comment
    Loop1_end: I2 += M0; 
...
Esc_mid:I2 = B2;      //I2 holds the address output array
	...
	R7 = R7 >>> 1 || I1 += M1 || NOP;
	M3 = R7;      //M3 holds twiddle factor offset.
	M0 = 8;
	I1 += 4;
	...
	A1 = R3.L * R6.H, A0 = R3.L * R6.L  || [I2++M0] = R4;
	R2 = PACK(R2.L, R2.H) || R5 = [I1];
	R6.H = (A1 += R3.H *R6.L), R6.L = (A0 -= R3.H * R6.H)   || R3 = [I3++M3];
	R5 = R5 +|+ R6, R6 = R5 -|- R6 (CO)                     || R4 = [I2];
	R5 = PACK(R5.L, R5.H)  || [I0++M0] = R2;
