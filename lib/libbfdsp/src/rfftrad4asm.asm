 ...
 ******************************************************************************
  File Name    : rfftrad4.asm
  Module Name  : one dimensional radix 4 forward FFT for real data.
  Label name   :  __rfftrad4_fr16
  
  Description  : The assembly function implements the radix4 FFT DIF algorithm. The first 
		 stage of the FFT which can be done separately is also done along with 
		 the middle stages since there is no significant reduction in the cycle 
		 count even if we do so. This is mainly because of the inability to use 
		 the multi-instructions effectively when we do the first stage separately.
		 ...
		 For FFT lengths less than 3*MAX_N/4, the same twiddle factor array can 
		 be used by choosing the stride factor appropriately, i.e., for an N-point 
		 FFT where N < MAX_N, the stride factor is MAX_N/N. For FFT length equal 
		 to MAX_N the stride factor is unity. The input and the output arrays are 
		 in normal order. Input data is scaled by 4 before each stage of radix 4 
		 fft including the first stage to avoid overflow (Static scaling)

 
  Registers Used : R0 to R7, P0 to P2, P5, M0 to M3, I0 to I3, B0 & B1
  ...
  Cycle count   :   1676 cycles (FFT length - 64)
...
__rfftrad4_fr16:
    ...
    A1 = A0 = 0 || [FP - 16] = R1;              // Store address of temporary buffer
    B0 = R2;                                    // Address of the output
    R1 = A1, R0 = A0 || R4 = [FP+24];           // Stride factor
    L0 = 0;                                     // Initialize L registers
    L1 = 0;                                     //          -do-
    L2 = 0;                                     //          -do-
    L3 = 0;                                     //          -do-
    R4 = R4 << 1 || R2 = [FP+20];               // Compute (Stride factor << 1)
    B1 = R2;                                    // Address of the twiddle factor array
    P5 = R3;                                    // Save FFT length
    
    [FP - 12] = R4 || R0.L = W[I0++];
    R2 = 1;
    R1 = R0 >>> 2 (V) || R0.L = W[I0++] || [FP - 4] = R2;   // [FP - 4] has the no. of groups in the first stage
    LSETUP (ST_CP, ST_CP) LC0 = P5;             // Copy input array to temporary buffer
ST_CP:  R1 = R0 >>> 2 (V) || [I1++] = R1 || R0.L = W[I0++]; // Scale the inputs by 4 to avoid overflow

//*************************************** FIRST STAGE*****************************************************

    CC = R3 <= 4 (IU);                      // If State > 4 do LOOP, else goto LAST_STAGE
    IF CC JUMP LAST_STAGE;
    R0 = [FP - 12];
    R2 = R3 << 2 || [FP - 8] = R0;
    M3 = R2;
    R1 = R3 >> 2;                           // Compute Group size = (State >> 2)
    P0 = R1;                                // Initialize butterfly loop counter
    R2 = [FP - 16] || NOP;                  // Offset =4*Group*Group size offset
    I0 = R2;
    I1 = R2;                                // Effective address for fetching the butterfly inputs
    M1 = R3;                                // Initialize Group size offset
    I2 = B1;                                // Save twiddle factor address
    I3 = 0;                                 // Initialize Butterfly count for each group
    I0 -= 4;
    MNOP || I0 += M3;
    LSETUP (ST_BFLY1, END_BFLY1) LC1 = P0;      // Do butterfly loop
ST_BFLY1:   R4 = [I1 ++ M1];                    // Fetch the input of each butterfly in the group   
	    ...
	    R0 = R0 +|+ R6 || I0 -= M3 || R7 = [I1 ++ M1];  
	    R2 = R0 +|+ R7 || I1 -= M3;         // Butterfly Offset
	    R2 = R2 >>> 2 (V) || I0 += 4 || R0 = [I2 ++ M2];
	    ...
	    R7.H = R5.L*R2.H, R7.L = R5.H*R2.H || I2 -= M0;
	    R0 = R6 -|+ R7 || I1 += 4;
	    R0 = R0 >>> 2 (V) || I3 += 4;
END_BFLY1:  [I0 ++ M1] = R0 || R0 = [FP - 8];   // Store the fourth o/p of the Butterfly
	P2 = 1;                                 // Increment the Group count by one
...
ST_MIDDLE:R0 = P2;                              // Offset computation for fetching the... 
	R0 = R0 << 2 || R2 = [FP - 16];         // butterfly inputs of every group...
	I3 = 0;                                 // Initialize Butterfly count for each group
	R1 = R0.L*R3.L (IS) || R0 = [FP - 8];   // Offset =4*Group*Group size offset
	R1 = R1 + R2;                           // Add Start address of each group and the Offset address
	I0 = R1;
	I1 = R1;                                // Effective address for fetching the butterfly inputs
	I2 = B1;                                // Save twiddle factor address
	I0 -= 4;
	I0 += M3;
...
END_MIDDLE:P2 += 1;                             // Increment the Group count by one
	R3 = R3 >> 2 || R7 = [FP - 4];          // Update the State count & Compute no. of Groups in the stage
	R7 <<= 2;
	[FP - 4] = R7;
	JUMP STAGE_LOOP;
...
ST_LAST_STAGE:  R4 = [I1++] || NOP;             // Fetch the Butterfly inputs
		R5 = [I1++];
