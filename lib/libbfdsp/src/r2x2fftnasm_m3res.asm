 ...
 ******************************************************************************
  File Name    : r2x2fftnasm.asm
  Module Name  : Two dimensional radix 2x2 forward FFT for real data.
  ...
  Description  : This file contains the code for the implementation of FFT. The
		 ...
		 point of view, the whole computation has been divided in three
		 ...
		 and output both the data are complex and 16 bit and represented
		 in 1Q15 format.

		 The input and the output are in normal order.
  ...
  Performance:
	    The code size   =   792  Bytes.
	    The cycle count for 8 x 8 input    =   2868 cycles. 
...
/************************ Program Section ****************************************/
//The name for the Radix 2x2 FFT label is __rfft2d_fr16.
...
//Copy the input data to output buffer so that input values can be preserved.
    ...
    Copy_strt:R1 = R0 >>> 2 (V) || R0.L = W[I0++]; 
    Copy_end: [I1++] = R1;  //Copy the data
...
/***************** Implementation of First Stage **********************************/
	...
	R1 = R1 + R2 (NS)  || I1 += M1;
	M0 = R1;                 //I1 stores the Odd-even address of butterfly.
	I3 = B2;                 // M0 = offset2 +le
	P3 = B3;                 //Starting address of Twiddle factor
	P5 = P0 << 2;            //wst * 4 to get correct byte in buffer.
	I3 += M0;                //I3 stores the Odd-Odd address of butterfly.
...
//is stored back. Then value of out[offset2+le] is also stored back.
	...
	R4.H = (A1 += R4.H * R3.L), R4.L = (A0 -= R4.H * R3.H) || I2 += M0;

	R5 = P3;
	R5.L = R5.L << 1 || I2 -= M0 || [I2] = R4;               //2*le
...
//I3 as offset2+le.
	...
	I2 += 4 || R4 = [I3++];  //I2 and I3 are incremented by 1
...
//This code Bit reverses the output array.
	...
	If CC Jump Swap;
	CC = R3 == R4;         //k1 == BR[k1].
	If !CC  Jump Copy_col_end;
	CC = R5 < R6;          //k2 < BR[k2].
	If !CC Jump Copy_col_end;
    Swap: A0 = R5;             //If satisfy all the conditions then swap the data.
	R5.L = (A0 += R3.L * R0.L) (IS);
	M2 = R5;               //M2 = k1*n + k2
	A0 = R6;
	R6.L = (A0 += R4.L * R0.L)(IS) || I2 += M2; 
