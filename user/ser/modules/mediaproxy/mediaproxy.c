/* $Id: mediaproxy.c,v 1.2 2004/11/24 01:33:21 pchunt Exp $
 *
...
{
	static char buf[16] = "unknown";
	static str notfound = {buf, 7};
...
{
	static char buf[16] = "unknown-agent";
	static str notfound = {buf, 13};
	str block, server;
	char *ptr;
...
UseMediaProxy(struct sip_msg* msg, char* str1, char* str2)
{
	...
	if (request) {
		toDomain = getDestinationDomain(msg);
