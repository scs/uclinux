...
rtp_session_recvm_with_ts (RtpSession * session, uint32_t user_ts)
{
	...
	if (session->permissive || session->rtp.jittctl.jitt_comp_ts==0)
...
#if 0
	if (session->eventqs==NULL) ortp_event_destroy(ev);
	else{
		OList *it;
		int i;
		for(i=0,it=session->eventqs;it!=NULL;it=it->next,++i){
			if (i==0) ortp_ev_queue_put((OrtpEvQueue*)it->data,ev);
			else  ortp_ev_queue_put((OrtpEvQueue*)it->data,ortp_event_dup(ev));
		}
	}
#else
	{
 ...
 *
 *	Enable or disable the "rtp symmetric" hack: after the first packet is
 *	received, the source address of the packet is set to be the destination
 *	address for all nexts packets.
**/
void
