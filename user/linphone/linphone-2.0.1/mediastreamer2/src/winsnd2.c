...
static void winsnd_write_process(MSFilter *f){
	...
	if (d->nbufs_playing==0)
		ms_warning("no buffer currently playing (q.m_count=%i), (playing: %i)", f->inputs[0]->q.q_mcount,  d->nbufs_playing);
	...
	while((m=ms_queue_get(f->inputs[0]))!=NULL){
		for(i=0;i<d->stat_minimumbuffer;++i){
			...
			if (hdr->dwUser==0){
				...
				ms_debug("waveOutPrepareHeader() done");
	...
	if ( d->stat_output > 150
		&& ((d->stat_notplayed*100)/d->stat_output) >10
		&& d->stat_minimumbuffer<15)
	{
		d->stat_minimumbuffer+=6;
		ms_message("Increasing sound card number of write buffer to %i", d->stat_minimumbuffer);
		d->stat_notplayed = 0; /* reset number... */
	}
	else if ( d->stat_output > 150
		&& ((d->stat_notplayed*100)/d->stat_output) >1
		&& d->stat_minimumbuffer<18)
	{
		d->stat_minimumbuffer+=3;
		ms_message("Increasing sound card number of write buffer to %i", d->stat_minimumbuffer);
		d->stat_notplayed = 0; /* reset number... */
	}
