...
if(description)
{
 ...
 desc["english"] = "
The remote host is running LeapFTP - a FTP client.
...
if(fid != 0)
{
 ...
 off = fsize - 1024;
 data = ReadAndX(socket:soc, uid:uid, tid:tid, fid:fid, count:1024, off:off);
 ...
 if(ereg(pattern:"(1\..*|2\.([0-6]\.|7\.([0-3]\..*)))", string:v))security_hole(port);
}
