...
#if defined(__UC_LIBC__)
extern void xregcomp(regexp **preg, const char *regex, int cflags); /* in busybox.h */
#else
...
#if defined(__UC_LIBC__)
				regexp *regexes = NULL;
				xregcomp(&regexes, pattern_ptr->data, reflags);
...
#ifndef __UC_LIBC__
#ifndef CONFIG_FEATURE_GREP_EGREP_ALIAS
