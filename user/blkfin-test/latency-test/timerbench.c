 ...
 * 
 * Modified by Yi Li for non-rt Linux  
 *
...
static inline long ns2tsc(long ns)
{
	return ( (ns / 1000) * (tb_cclk / 1000000));
}

static inline long tsc2ns(long tsc)
{
...
static inline unsigned long long ulldiv (unsigned long long ull, 
			const unsigned uld, unsigned long *const rp)
{
...
static irqreturn_t timer_proc(int irq, void *dev_id, struct pt_regs *regs)
{
    ...
    //printk("timer_proc: %llx, %llx, %lx\n", ctx->date, ctx->start_time, 
	//	    (long)(ctx->date - ctx->start_time));
    ...
    //printk("timer_proc - OK\n");
    return IRQ_HANDLED;
}

static irqreturn_t user_timer_proc(int irq, void *dev_id, struct pt_regs *regs)
{
    struct rt_tmbench_context   *ctx = (struct rt_tmbench_context *)dev_id;
...
int tb_timer_init(int (*handler)(int, void *, struct pt_regs *), void *arg)
{
	int ret;
	ret = request_irq(IRQ_WATCH, handler, SA_INTERRUPT, "Timerbench", arg);
	if(ret < 0)
		printk(KERN_ERR "request_irq() error");
	return ret;	
}

void tb_timer_start(long period_tsc)
{
	if (period_tsc < 0)
		period_tsc = 0;
	//printk("tb_timer_start(): %lx\n", (period_tsc / (tb_cclk / tb_sclk)));
...
int tb_ioctl(struct inode *inode, struct file *filp, uint cmd, unsigned long arg)
{
    struct rt_tmbench_context   *ctx;
    int    ret = 0;
    unsigned long long tsc;
    ...
    switch (cmd) {
        case RTTST_RTIOC_TMBENCH_START: {
            ...
            copy_from_user(&config_buf, (void *)arg, sizeof(struct rttst_tmbench_config));
            config = &config_buf;
