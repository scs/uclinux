...
while ((c = getopt(argc, argv, "vth?tr:a:c:b:")) > 0) {
  ...
  if(usetrigger)
    ...
    if(filename) {
	    ...
	    (void) close( fd );
	    printf("*******************************************************************************\n");
	    printf("Saved: %s \nSize : %d\n",filename, IMAGESIZE+sizeof(bmphead));
	    printf("*******************************************************************************\n");
  }
