...
int linphone_set_audio_offer(sdp_context_t *ctx)
{
	...
	while(elem!=NULL){
		...
		if (linphone_core_check_payload_type_usability(lc,codec) && payload_type_enabled(codec)){
			...
			}else if (strcasecmp(codec->mime_type,"iLBC")==0){
...
SupportLevel linphone_payload_is_supported(LinphoneCore *lc, sdp_payload_t *payload,RtpProfile *local_profile,RtpProfile *dialog_profile)
{
	int localpt;
	SupportLevel ret;
	...
	if (localpt>=0 && localpt <128 ){
		...
		if (strcmp(rtppayload->mime_type,"telephone-event")!=0){
			if (!linphone_core_check_payload_type_usability(lc,rtppayload) ){
...
int linphone_accept_audio_offer(sdp_context_t *ctx,sdp_payload_t *payload)
{
	...
	supported=linphone_payload_is_supported(lc,payload,lc->local_profile,remote_profile);
...
int linphone_read_audio_answer(sdp_context_t *ctx,sdp_payload_t *payload)
{
	...
	supported=linphone_payload_is_supported(lc, payload,lc->local_profile,call->profile);
	if (supported==Unsupported) {
