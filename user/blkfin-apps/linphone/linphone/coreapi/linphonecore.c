...
LinphoneCallLog * linphone_call_log_new(LinphoneCall *call, char *from, char *to){
	...
	cl->lc=call->core;
	cl->dir=call->dir;
	cl->start_date=ms_strdup(ctime(&call->start_time));
	cl->from=from;
	cl->to=to;
	return cl;
}
...
char * linphone_call_log_to_str(LinphoneCallLog *cl){
	...
	return ortp_strdup_printf(_("%s at %sFrom: %s\nTo: %s\nStatus: %s\nDuration: %i mn %i sec\n"),
...
void linphone_call_log_destroy(LinphoneCallLog *cl){
	if (cl->start_date!=NULL) ms_free(cl->start_date);
...
net_config_read (LinphoneCore *lc)
{
	...
	tmp=lp_config_get_int(config,"net","use_stun",0);
	linphone_core_enable_stun(lc,tmp);
	...
	linphone_core_use_nat_address(lc,tmp);		
}
...
static PayloadType experimental_theora={
	.type=PAYLOAD_VIDEO,
	.clock_rate=90000,
	.bits_per_sample=0,
	.zero_pattern=NULL,
...
linphone_core_init (LinphoneCore * lc, const LinphoneCoreVTable *vtable, const char *config_path, void * userdata)
{
	...
	rtp_profile_set_payload(&av_profile,96,&experimental_theora);
...
const MSList *linphone_core_get_audio_codecs(LinphoneCore *lc)
{
	return lc->codecs_conf.audio_codecs;
}

const MSList *linphone_core_get_video_codecs(LinphoneCore *lc)
{
...
void linphone_core_get_local_ip(LinphoneCore *lc, const char *dest, char *result){
	const char *loc_addr;
	char *tmp=NULL;
	loc_addr=linphone_core_get_nat_address_if_used(lc);
	if (loc_addr!=NULL){
		strncpy(result,loc_addr,LINPHONE_IPADDR_SIZE);
		return;
	}
	if (linphone_core_stun_enabled(lc)) {
...
void linphone_core_enable_stun(LinphoneCore *lc, bool_t val){
	lc->net_conf.stun_enabled=val;
}

bool_t linphone_core_stun_enabled(const LinphoneCore *lc){
	return lc->net_conf.stun_enabled;
}
...
static void apply_nat_settings(LinphoneCore *lc){
	...
	if (lc->net_conf.use_nat && addr!=NULL && strlen(addr)>0){
...
void linphone_core_use_nat_address(LinphoneCore *lc, bool_t val){
	lc->net_conf.use_nat=val;
	apply_nat_settings(lc);
}

bool_t linphone_core_nat_address_used(const LinphoneCore *lc){
	return lc->net_conf.use_nat;
}
...
const char *linphone_core_get_nat_address(LinphoneCore *lc)
{
	return lc->net_conf.nat_address;
}

const char *linphone_core_get_nat_address_if_used(LinphoneCore *lc){
	if (lc->net_conf.use_nat && lc->net_conf.nat_address!=NULL && strlen(lc->net_conf.nat_address)>0) return lc->net_conf.nat_address;
	else return NULL;		
}
...
void net_config_uninit(LinphoneCore *lc)
{
	...
	lp_config_set_int(lc->config,"net","use_stun",config->stun_enabled);
