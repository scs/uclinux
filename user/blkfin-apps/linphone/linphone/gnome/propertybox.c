...
void net_section_fill(NetSection *sec,LinphoneCore *lc)
{
	...
	use_nat=linphone_core_nat_address_used(lc);
	...
	if (!use_nat)
		gtk_toggle_button_set_active(
			GTK_TOGGLE_BUTTON(lookup_widget(get_property_box()->prop,"use_stun")),
			linphone_core_stun_enabled(lc));
                                        ...
                                        gpointer         user_data)
{
	...
	linphone_core_use_nat_address(get_core(),use_nat);
	property_box_changed();
...
void net_section_apply(NetSection *sec,LinphoneCore *lc)
{
	...
	linphone_core_use_nat_address(lc,use_nat);
	g_free(name);
	...
	g_free(name);
	linphone_core_enable_stun(lc,use_stun);
