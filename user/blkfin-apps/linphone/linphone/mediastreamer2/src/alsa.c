...
static int alsa_read(snd_pcm_t *handle,unsigned char *buf,int nsamples)
{
	...
	if (err<0) {
		if (err==-EPIPE){
			...
			err=snd_pcm_readi(handle,buf,nsamples);
			if (err<0) ms_warning("alsa_card_read: snd_pcm_readi() failed:%s.",snd_strerror(err));
		}else if (err!=-EWOULDBLOCK){
			ms_warning("alsa_card_read: snd_pcm_readi() failed:%s.",snd_strerror(err));
		}
	}
	return nsamples;
}
...
void alsa_read_process(MSFilter *obj){
	...
	while (alsa_can_read(ad->handle,samples)){
		om=allocb(size,0);
		if (alsa_read(ad->handle,om->b_wptr,samples)!=samples){
