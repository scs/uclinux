...
static void sdl_out_init(MSFilter  *f){
	...
	obj->size=MS_VIDEO_SIZE_CIF;
	obj->local_size=MS_VIDEO_SIZE_CIF;
...
static void sdl_out_uninit(MSFilter *f){
	...
	if (s->small!=NULL) freemsg(s->small);
	ms_free(s);
}
...
static mblk_t * resize_yuv_small(unsigned char *pict, int w, int h, int scale){
	...
	mblk_t *small=allocb(smallpict_sz,0);
	smallpict=small->b_wptr;
	small->b_wptr+=smallpict_sz;
	...
	while (f->inputs[1]!=NULL && (inm1=ms_queue_get(f->inputs[1]))!=NULL){
		...
		if (!obj->lsize_init){
			...
			if (bsize<(MS_VIDEO_SIZE_CIF_W*MS_VIDEO_SIZE_CIF_H*3/2)){
				/*surely qcif ?*/
				obj->local_size=MS_VIDEO_SIZE_QCIF;
		...
		if  (obj->format==MS_YUV420P){
			...
			obj->small=resize_yuv_small(inm1->b_rptr,obj->local_size.width,obj->local_size.height,obj->scale_factor);
			fill_overlay_at_pos(obj->overlay,obj->small,smallrect.x, smallrect.y, smallrect.w, smallrect.h);
			freemsg(inm1);
		}else {
