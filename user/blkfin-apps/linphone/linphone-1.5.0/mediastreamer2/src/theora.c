...
static int enc_set_fps(MSFilter *f, void *data){
	...
	s->tinfo.keyframe_frequency_force=(*fps)*20;
	return 0;
}
...
static int enc_set_br(MSFilter *f, void*data){
	...
	fps=s->tinfo.fps_numerator;
	s->tinfo.target_bitrate=(float)br/1.2;
	s->tinfo.keyframe_data_target_bitrate=br;
	/*those default settings would need to affined*/
	if (br>=256000){
...
static void enc_process(MSFilter *f){
	...
	int elapsed=timems-s->start_time;
	while((im=ms_queue_get(f->inputs[0]))!=NULL){
		/*for the firsts frames only send theora packed conf*/
		om=NULL;
		if (elapsed<1000){
			/* during the first second, only send packed conf twice */
			if (elapsed>=s->conf_time){
				s->conf_time+=500;
				om=dupmsg(s->packed_conf);
				ms_debug("sending theora packed conf");
