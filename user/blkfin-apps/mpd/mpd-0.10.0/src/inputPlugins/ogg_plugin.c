...
#ifdef HAVE_MPD_IVORBIS
# include "../tremor/ivorbisfile.h"
# define T_SCALE        1000     /* milliseconds */
#else /* !USE_MPD_VORBIS */
//# include <vorbis/vorbisfile.h>
# define T_SCALE        1        /* seconds */
...
#define OGG_CHUNK_SIZE 4096
	...
	dc->totalTime = ov_time_total(&vf,-1)/T_SCALE;
	...
	while(!eof) {
		if(dc->seek) {
			if(0 == ov_time_seek_page(&vf,dc->seekWhere*T_SCALE)) {
 ...
 ret = ov_read(&vf, chunk+chunkpos, OGG_CHUNK_SIZE-chunkpos, &current_section);
#if 0
		ret = ov_read(&vf, chunk+chunkpos, 
				OGG_CHUNK_SIZE-chunkpos,
#ifndef HAVE_MPD_IVORBIS
				OGG_DECODE_USE_BIGENDIAN,
				2, 1,
#endif /* !HAVE_MPD_IVORBIS */
				&current_section);
#endif
	...
	if(!dc->stop && chunkpos > 0) {
		sendDataToOutputBuffer(cb, NULL, dc, inStream->seekable,
				chunk, chunkpos,
				ov_time_tell(&vf)/T_SCALE, bitRate, replayGainInfo);
	}
...
//		printf("1 .OggTagDup %s\r\n",file);
	
	ret = oggCommentsParse(ov_comment(&vf,-1)->user_comments);
//		printf("2.  OggTagDup %s\r\n",file);

	if(!ret) ret = newMpdTag();
#ifdef HAVE_MPD_IVORBIS
	ret->time = (int)(ov_time_total(&vf,-1)/T_SCALE);
#else /* !HAVE_MPD_IVORBIS */
	ret->time = (int)(ov_time_total(&vf,-1)+0.5);
#endif /* !HAVE_MPD_IVORBIS */
//		printf("3. OggTagDup %s\r\n",file);
	ov_clear(&vf);
//		printf("4. OggTagDup %s\r\n",file);
