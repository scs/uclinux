...
static inline signed long audio_linear_dither(
		mad_fixed_t sample, struct audio_dither *dither) {
	unsigned int scalebits;
	mad_fixed_t output, mask, random;
	register mad_fixed_t * d_tmp = dither->error;

	sample += d_tmp[0] - d_tmp[1] + d_tmp[2];
...
static inline int decodeNextFrameHeader(mp3DecodeData * data, MpdTag ** tag) {
...
static inline int decodeNextFrame(mp3DecodeData * data) {
	if((data->stream).buffer==NULL || (data->stream).error==MAD_ERROR_BUFLEN) {
		if(fillMp3InputBuffer(data) < 0) {
			return DECODE_BREAK;
		}
...
int mp3Read(mp3DecodeData * data, OutputBuffer * cb, DecoderControl * dc) {
	...
	switch(data->muteFrame) {
		...
		{
			unsigned int *s0 = &(data->synth).pcm.samples[0][0];
			unsigned int *s1 = &(data->synth).pcm.samples[1][0];
			for(i=(data->synth).pcm.length;i!=0;--i) {
				mpd_sint16 * sample;

				sample = (mpd_sint16 *)data->outputPtr;	
				*sample = (mpd_sint16) audio_linear_dither(
						*(s0++), &(data->dither));
				data->outputPtr+=2;
