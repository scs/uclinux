...
//		printf("URLin= %s , Songtypein=%d \r\n", url,type);
	...
	if(song->type == SONG_TYPE_FILE) {
                InputPlugin * plugin;
//		printf("getSongUrl(song)= %s , song->time=%d \r\n",getSongUrl(song),&(song->mtime));
		if((plugin = isMusic(getSongUrl(song), &(song->mtime)))) 
			{
		        song->tag = plugin->tagDupFunc(	rmp2amp(utf8ToFsCharset(getSongUrl(song))));
            }
