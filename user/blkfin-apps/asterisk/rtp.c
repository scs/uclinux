...
static int ast_rtp_raw_write(struct ast_rtp *rtp, struct ast_frame *f, int codec)
{
	...
	/* Blackfin chokes on non-aligned uint writes so use memcpy 
           instead */ 
	/*
	...
	*/
	{
	  unsigned int tmp;
	  tmp = htonl((2 << 30) | (codec << 16) | (rtp->seqno++) | (mark << 23));
	  memcpy(&rtpheader[0], &tmp, sizeof(unsigned int));
	  tmp = htonl(rtp->lastts);
	  memcpy(&rtpheader[1], &tmp, sizeof(unsigned int));
	  tmp = htonl(rtp->ssrc); 
	  memcpy(&rtpheader[2], &tmp, sizeof(unsigned int));
	}
