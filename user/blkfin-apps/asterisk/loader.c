...
// DR: emulate dl functions with static libs using GNU libtool ltdl functions

#ifndef RTLD_LAZY
#define RTLD_LAZY 0
#endif

static void *dlopen(char module_name[], int flags) {
	char module_name_a[256];
	lt_dlhandle handle;
	char *dot;
	...
	handle = lt_dlopen(module_name_a);
...
static void *dlsym(char module_name[], void *handle, char sym_name[]) {
	char module_name_a[256];
	char name[256];
	char *dot;
	...
	sprintf(name,"%s_LTX_%s",module_name_a,sym_name);
...
int ast_load_resource(char *resource_name)
{
	...
	//}
	m->lib = dlopen(resource_name, flags);
	if (!m->lib) {
	...
	m->load_module = dlsym(m->resource, m->lib, "load_module");
	if (m->load_module == NULL)
		m->load_module = dlsym(m->resource, m->lib, "_load_module");
	if (!m->load_module) {
		ast_log(LOG_WARNING, "No load_module in module %s\n", fn);
		errors++;
	}
	m->unload_module = dlsym(m->resource, m->lib, "unload_module");
	if (m->unload_module == NULL)
		m->unload_module = dlsym(m->resource, m->lib, "_unload_module");
	if (!m->unload_module) {
		ast_log(LOG_WARNING, "No unload_module in module %s\n", fn);
		errors++;
	}
	m->usecount = dlsym(m->resource, m->lib, "usecount");
	if (m->usecount == NULL)
		m->usecount = dlsym(m->resource, m->lib, "_usecount");
	if (!m->usecount) {
		ast_log(LOG_WARNING, "No usecount in module %s\n", fn);
		errors++;
	}
	m->description = dlsym(m->resource, m->lib, "description");
	if (m->description == NULL)
		m->description = dlsym(m->resource, m->lib, "_description");
	if (!m->description) {
		ast_log(LOG_WARNING, "No description in module %s\n", fn);
		errors++;
	}
	m->key = dlsym(m->resource, m->lib, "key");
	if (m->key == NULL)
		m->key = dlsym(m->resource, m->lib, "_key");
	if (!m->key) {
		ast_log(LOG_WARNING, "No key routine in module %s\n", fn);
		errors++;
	}
	m->reload = dlsym(m->resource, m->lib, "reload");
	if (m->reload == NULL)
		m->reload = dlsym(m->resource, m->lib, "_reload");
	if (!m->key || !(key = m->key())) {
