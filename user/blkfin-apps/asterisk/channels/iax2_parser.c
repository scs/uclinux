...
static void dump_int(char *output, int maxlen, void *value, int len)
{
	...
	if (len == (int)sizeof(unsigned int)) {
		/* memcpy to ensure we get an int with correct address
		   alignment for this CPU */
		memcpy(&ui, value, sizeof(unsigned int));
		snprintf(output, maxlen, "%lu", (unsigned long)ntohl(ui));
	}
	else
...
static void dump_short(char *output, int maxlen, void *value, int len)
{
	...
	if (len == (int)sizeof(unsigned short)) {
		/* memcpy to local ensures that short is on the 
		   correct memory alignment for this CPU */
		memcpy(&s, value, sizeof(unsigned short));
		snprintf(output, maxlen, "%d", ntohs(s));
	}
	else
	...
	while(datalen >= 2) {
		...
		case IAX_IE_CAPABILITY:
			if (len != (int)sizeof(unsigned int)) {
				...
				memcpy(&ui, data + 2, sizeof(unsigned int));
				ies->capability = ntohl(ui);
			}
			break;
		case IAX_IE_FORMAT:
			if (len != (int)sizeof(unsigned int)) {
		...
		case IAX_IE_VERSION:
			if (len != (int)sizeof(unsigned short)) {
				...
				memcpy(&us, data + 2, sizeof(unsigned short));
				ies->version = ntohs(us);
			break;
		case IAX_IE_ADSICPE:
			if (len != (int)sizeof(unsigned short)) {
