...
#include <unistd.h>
#ifdef USE_MPD_FLAC
# include "../flac/include/FLAC/seekable_stream_decoder.h"
...
static int flacSendChunk(FlacData * data);
...
static FLAC__bool flacEOF(
		const FLAC__SeekableStreamDecoder * flacDec, void * fdata)
{
	FlacData * data = (FlacData *) fdata;
...
static int flacSendChunk(FlacData * data) {
	switch(sendDataToOutputBuffer(data->cb, NULL, data->dc, 1, data->chunk,
 			data->chunk_length, data->time/1024, data->bitRate,
 			data->replayGainInfo)) 
	{
...
static FLAC__StreamDecoderWriteStatus flacWrite(
		const FLAC__SeekableStreamDecoder *dec,
                const FLAC__Frame *frame, const FLAC__int32 * const buf[], 
                void * vdata) 
{
	FlacData * data = (FlacData *)vdata;
	FLAC__uint32 samples = frame->header.blocksize;
	register unsigned char *uc0, *uc1, *fchunk;
	register unsigned chunk_len;
	unsigned timeChange, i;
	FLAC__uint64 newPosition = 0;

	timeChange = (samples * 1024)/frame->header.sample_rate;
	data->time += timeChange;

	FLAC__seekable_stream_decoder_get_decode_position(dec,&newPosition);

	/*  this is a waste of time to calculate for FLAC on the ipod
 	if(data->position) {
...
#define send_and_reset() do { \
	if(chunk_len==0) { \
		data->chunk_length = chunk_len = FLAC_CHUNK_SIZE; \
	...
	chunk_len = FLAC_CHUNK_SIZE - data->chunk_length;
	fchunk = &data->chunk[data->chunk_length];
	uc0 = (unsigned char *)&buf[0][0];
	uc1 = (unsigned char *)&buf[1][0];
	for(i = frame->header.blocksize; i != 0; --i) {
		send_and_reset();		
		*(fchunk++) = *(uc0++);
	...
	data->chunk_length = FLAC_CHUNK_SIZE - chunk_len;
	return FLAC__STREAM_DECODER_WRITE_STATUS_CONTINUE;
}
	...
	*vorbisCommentFound = 0;
	...
	do {
		...
		if(block->type == FLAC__METADATA_TYPE_VORBIS_COMMENT) {
			...
			ret->time = block->data.stream_info.total_samples /
					block->data.stream_info.sample_rate;
		}
