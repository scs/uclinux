...
#define clearUpdatePid() do { \
		mpm_set_id(MPM_UPDATE,0); \
		mpm_free(MPM_UPDATE); \
	} while (0)

static void update_directory_stats () {
	struct stat st;
...
int isUpdatingDB() {
 	if(mpm_get_id(MPM_UPDATE)>0) {
		return directory_updateJobId;
	}
	return 0;
}

void directory_sigChldHandler(pid_t pid, int status) {
 	if(mpm_get_id(MPM_UPDATE)==pid) {
...
int update_task (void *pl) {
	...
	if(pl) {
		...
		while(node) {
			...
			default:
				if (pl) freeList(pl);
				return DIRECTORY_UPDATE_EXIT_ERROR;
				break;
			}
			node = node->nextNode;
		}
	} else if((dbUpdated = updateDirectory(mp3rootDirectory))<0) {
		if (pl) freeList(pl);
		return DIRECTORY_UPDATE_EXIT_ERROR;
	}
	if(!dbUpdated) { 
		if (pl) freeList(pl);
		return DIRECTORY_UPDATE_EXIT_NOUPDATE;
	}
	...
	return DIRECTORY_UPDATE_EXIT_UPDATE;
}
...
int updateInit(FILE * fp, List * pathList) {
	...
	rc = mpm_spawn(MPM_UPDATE, (void*)pathList);
	if (rc<=0) {
	...
	DEBUG("updateInit: mpm_spawned()'d update thread for update job id %i\n",
...
/* findSubDirectory - no need to be thread-safe (strtok) -- eric 20040921 */
...
int writeDirectoryDB() {
	FILE * fp;
	char tmp_db[MAXPATHLEN+1];

	snprintf(tmp_db,MAXPATHLEN,"%s.%05d.%ld",
			directory_db,getpid(),time(NULL));

	deleteEmptyDirectoriesInDirectory(mp3rootDirectory);
	sortDirectory(mp3rootDirectory);

	while(!(fp=fopen(tmp_db,"w")) && errno==EINTR);
	if(!fp) {
		unlink(tmp_db);
		return -1;
	}
...
/* getSongDetails: no need to be thread-safe (strtok) -- eric 20040921 */
Song * getSongDetails(char * file, char ** shortnameRet, 
		Directory ** directoryRet)
{
	void * song = NULL;
