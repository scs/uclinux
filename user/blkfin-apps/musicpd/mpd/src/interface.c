...
static int interface_max_connections = 0;
...
/* ignore the static variable here, it's thread-safe since only the
 * main process prints to (fd != 1 && fd != 2) */
...
void printInterfaceOutBuffer(Interface * interface) {
	...
	if(interface->bufferList) {
		...
		if(interface->outputBufferSize>
 		...
 		{
			ERROR("interface %i: output buffer size (%zu) is "
					"larger than the max (%zu)\n",
					interface->num,
