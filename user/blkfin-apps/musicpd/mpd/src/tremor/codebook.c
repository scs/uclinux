...
int decode_map(codebook *s, oggpack_buffer *b, ogg_int32_t *v, int point){
  ...
  case 2:{
    ...
    const int mask=(1<<s->q_pack)-1;
    if(s->q_bits<=8){
      for(i=s->dim;i!=0;--i){
	*(vidx++)=((unsigned char *)(s->q_val))[entry&mask];
        entry>>=s->q_pack;
      }
    }else{
      for(i=s->dim;i!=0;--i){
	*(vidx++)=((ogg_uint16_t *)(s->q_val))[entry&mask];
        entry>>=s->q_pack;
      }
    }
    break;
  ...
  case 3:{
    /* offset into array */
    register void *ptr=s->q_val+entry*s->q_pack;

    if(s->q_bits<=8){
      for(i=s->dim;i!=0;--i)
	*(vidx++)=*((unsigned char *)ptr++);
    }else{
      for(i=s->dim;i!=0;--i)
	*(vidx++)=*((ogg_uint16_t *)ptr++);
    }
    break;
  }
  default:
  ...
  /* we have the unpacked multiplicands; compute final vals */
  {
    ...
    vidx = &v[0];
    if(shiftM>0)
      for(i=s->dim;i!=0;--i,vidx++)
	*vidx = add + ((*vidx * s->q_del) >> shiftM);
    else
      for(i=s->dim;i!=0;--i,vidx++)
	*vidx = add + ((*vidx * s->q_del) << -shiftM);

    if(s->q_seq) {
      register ogg_int32_t *vidx_ = &v[0];
      vidx = &v[1];
      for(i=s->dim-1;i!=0;--i)
	*(vidx++) = *(vidx_++);
    }
