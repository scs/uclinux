...
/*
 *
 *    Rev:          $Id: getSinglePic_read.c,v 1.1 2007/12/14 09:09:34 mberner Exp $
 *    Revision:     $Revision: 1.1 $
 *    Source:       $Source: /cvs/ferag/BogenerkennungsImplementation/uclinux-dist/user/blkfin-apps/ppifcd-cgi-lcv/src/getSinglePic_read.c,v $
...
#else
	buffer = (unsigned char *)malloc(IMAGESIZE + sizeof(bmphead)); //1 byte per pixel
#endif
...
//for (tmpi = 0; tmpi < 100; tmpi++) {
	...
	for (i = 0; i < cnt; i++) {
		read(fd, buffer + sizeof(bmphead), IMAGESIZE);
	       	usleep(50000); /* Needed between two exposures in 
Snapshot mode */
	}
