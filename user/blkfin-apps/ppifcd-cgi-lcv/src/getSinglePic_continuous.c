...
/*
 *
 *    Rev:          $Id: getSinglePic_continuous.c,v 1.1 2007/12/14 09:09:34 mberner Exp $
 *    Revision:     $Revision: 1.1 $
 *    Source:       $Source: /cvs/ferag/BogenerkennungsImplementation/uclinux-dist/user/blkfin-apps/ppifcd-cgi-lcv/src/getSinglePic_continuous.c,v $
...
enum {
	...
	FRAME_ERROR = 4,	/* < Error                            */
	FRAME_BUSY = 5,		/* < Checked out to user space */
};
...
/* uCam ioctl commands */
#define FILL_BUFFERS 0
#define RESERVE_BUFFER 1
#define RELEASE_BUFFER 2
...
int getSinglePic(unsigned char *filename)
{
	printf("filename is %s", filename);
	int fd, fd_bgr, i, c, cnt, delay, board, trigger_strobe;
	...
	ioctl(fd, FILL_BUFFERS);	

	struct uCam_buffer *res_buf;
	/* warm-up: Read the raw image data from the PPI */
	for (i = 0; i < cnt; i++) {
		ioctl(fd, RESERVE_BUFFER, res_buf);
		ioctl(fd, RELEASE_BUFFER, res_buf);
	...
	printf("calling raw10to8bit\n");
	Raw10To8bit(res_buf->data, IMAGESIZE);	

	/* For CFA Sensors: */
        printf("calling cfa algo\n");
	unsigned char* bgr_buffer = cfa2bgr_DoBayerFiltering(res_buf->data, sizeof(bmphead_bgr), WIDTH, HEIGHT);
	...
	if (filename) {
		...
		printf
		    ("*******************************************************************************\n");
		printf("Saved BGR bmp: %s \nSize : %d\n", filename, IMAGESIZE_BGR + sizeof(bmphead_bgr));
		printf
		    ("*******************************************************************************\n");
	}
	...
	/* Close PPI */
	ioctl(fd, RELEASE_BUFFER, res_buf);
	close(fd); //would actually make ioctl(fd, RELEASE_BUFFEr, res_buf) unnecessary
