...
//	fprintf(stderr,"2.1 In adtsParse \r\n");
	...
	//		fprintf(stderr,"2.2 In adtsParse \r\n");
...
//		fprintf(stderr,"In AAC bytesPerFrame : %f\r\n",bytesPerFrame);
	}
		...
		size_t * retFileread, size_t * retTagsize)
{
	...
	//fprintf(stderr,"1.1 In initAacBuffer \r\n");
...
float getAacFloatTotalTime(char * file) {
	...
	printf("Adding File Wait \r\n");	
//	fprintf(stderr,"In getAacFloatTotal time\r\n");
...
int aac_decode(OutputBuffer * cb, DecoderControl * dc, char * path) {
	...
	AacBuffer b;
	InputStream inStream;
	fprintf(stderr,"The file to play : %s \r\n",path);
	fprintf(stderr,"0: In aac_decode\r\n");
	...
	fprintf(stderr,"In aacDecode :%d\r\n",sampleCount);
		...
		fprintf(stderr,"x[%d] sampleBufferLen = %d\r\n", x++,sampleBufferLen);
...
char * aac_mimeTypes[] = {"audio/aacp", NULL};
