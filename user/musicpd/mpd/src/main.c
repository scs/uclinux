...
void daemonize(Options * options, char ** argv) {
        if(options->daemon) {
		char * test;
                pid_t pid;
		fflush(NULL);
		if ( !(test = getenv("__MPD_D_1")) ) {
			if ((pid = vfork()) < 0) {
				ERROR("problems vfork'ing for daemon!\n");
				exit(EXIT_FAILURE);
			} else if (pid != 0) {
				_exit(EXIT_SUCCESS);
			}
			/* child */
			setenv("__MPD_D_1","1",1);
			execv(argv[0], argv);
			ERROR("Couldn't exec\n");
			_exit(EXIT_FAILURE);
		} else {
...
#if defined(HAVE_AUDIO) && !defined(HAVE_OSS)
...
#endif /* defined(HAVE_AUDIO) && !defined(HAVE_OSS) */
}
