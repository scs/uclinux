...
#ifdef IPV6READYLOGO
	if( !iface->UnicastOnly && (tv.tv_sec - iface->last_multicast < MIN_DELAY_BETWEEN_RAS))
	{	/* send multicast in 
		MIN_DELAY_BETWEEN_RAS - (tv.tv_sec - iface->last_multicast) sec
		 */
                clear_timer(&iface->tm);
                next=(double)(MIN_DELAY_BETWEEN_RAS-(tv.tv_sec-iface->last_multicast));
