...
int ac97_sport_open(size_t bufsize, size_t fragsize)
{
	...
	dev.rxbuf =
	    dma_alloc_coherent(NULL, bufsize * sizeof(struct ac97_frame), &addr,
			       0);
	dev.txbuf =
	    dma_alloc_coherent(NULL, bufsize * sizeof(struct ac97_frame), &addr,
			       0);
	dev.cmd_count =
	    dma_alloc_coherent(NULL, dev.fragcount * sizeof(int), &addr, 0);
...
#if L1_DATA_A_LENGTH != 0
	if (dev.tx_desc)
		l1_data_A_sram_free(dev.tx_desc);
	if (dev.rx_desc)
		l1_data_A_sram_free(dev.rx_desc);
#else
	if (dev.tx_desc)
		dma_free_coherent(NULL,
				  sizeof(struct dma_descriptor_block) *
				  dev.fragcount, dev.tx_desc, 0);
	if (dev.rx_desc)
		dma_free_coherent(NULL,
				  sizeof(struct dma_descriptor_block) *
				  dev.fragcount, dev.rx_desc, 0);
#endif

	if (dev.rxbuf)
		dma_free_coherent(NULL, dev.bufsize * sizeof(struct ac97_frame),
				  dev.rxbuf, 0);
	if (dev.txbuf)
		dma_free_coherent(NULL, dev.bufsize * sizeof(struct ac97_frame),
				  dev.txbuf, 0);
	if (dev.cmd_count)
		dma_free_coherent(NULL, dev.fragcount * sizeof(int),
				  dev.cmd_count, 0);
...
#ifdef AC97_SPORT_DEBUG
			printk(KERN_DEBUG
			       "ac97_sport: Sending initialization commands.\n");
#endif
			init_ac97();
		}
...
static void move_frames_to_pcm16(short *dest, struct ac97_frame *src,
				 size_t count)
{
	while (count--) {
		/* Left channel? */
...
static void move_frames_to_pcm32(__u32 * dest, struct ac97_frame *src,
				 size_t count)
{
...
ssize_t ac97_sport_get_pcm16_to_user(uint16_t * pcmdata, size_t count)
{
	...
	while (count > 0) {
		...
		tomove =
		    count >
		    dev.bufsize - dev.rx_tail ? dev.bufsize -
		    dev.rx_tail : count;

		if (tomove > 0) {
			move_frames_to_pcm16(pcmdata, dev.rxbuf + dev.rx_tail,
					     tomove);
			pcmdata += tomove;
			dev.rx_tail += tomove;
			count -= tomove;
		}
		if (dev.rx_tail >= dev.bufsize)
			dev.rx_tail = 0;
	}
	return cnt;
}
...
ssize_t ac97_sport_get_pcm32_to_user(uint32_t * pcmdata,
				     size_t /* sample_ */ count)
{
	int cnt = count;
	...
	while (count > 0) {
		...
		if (tomove > 0) {
			move_frames_to_pcm32(pcmdata, dev.rxbuf + dev.rx_tail,
					     tomove);
			pcmdata += tomove;
			dev.rx_tail += tomove;
			count -= tomove;
...
ssize_t ac97_audio_read(uint8_t * pcmdata, size_t len)
{
	...
	if (!rx_would_underflow(len)) {
		if (dev.stereo) {
			...
			samples_read =
			    ac97_sport_get_pcm32_to_user((uint32_t *) pcmdata,
							 samples_to_read);
			bytes_read = samples_read << 2;
		} else {
			samples_to_read = len >> 1;
			samples_read =
			    ac97_sport_get_pcm16_to_user((uint16_t *) pcmdata,
							 samples_to_read);
...
static void move_pcm16_to_frames(struct ac97_frame *dest, const __u16 * src,
				 size_t count)
{
	while (count--) {
...
static void move_pcm32_to_frames(struct ac97_frame *dest, const __u32 * src,
				 size_t count)
{
	while (count--) {
...
ssize_t ac97_sport_put_pcm16_from_user(const uint16_t * pcmdata, size_t count)
{
	...
	if ((dev.tx_head + count) >= dev.bufsize) {
		...
		if (count) {
			move_pcm16_to_frames(dev.txbuf + dev.tx_head, pcmdata,
					     count);
			pcmdata += count;
		}
		count = cnt - count;
...
ssize_t ac97_sport_put_pcm32_from_user(const uint32_t * pcmdata, size_t count)
{
	...
	if ((dev.tx_head + count) >= dev.bufsize) {
		...
		if (count) {
			move_pcm32_to_frames(dev.txbuf + dev.tx_head, pcmdata,
					     count);
			pcmdata += count;
		}
		count = cnt - count;
...
ssize_t ac97_audio_write(const uint8_t * pcmdata, size_t len)
{
	...
	if (!tx_would_overflow(len)) {
		if (dev.stereo) {
			...
			samples_written =
			    ac97_sport_put_pcm32_from_user((uint32_t *) pcmdata,
							   samples_to_write);
			bytes_written = samples_written << 2;
		} else {
			samples_to_write = len >> 1;
			samples_written =
			    ac97_sport_put_pcm16_from_user((uint16_t *) pcmdata,
							   samples_to_write);
