/*
 * File:         drivers/char/bfin_rtc.c
 * Based on:
 * Author:
 *
 * Created:
 * Description:  Real Time Clock interface of ADI21535 (Blackfin) for uCLinux
 *
 * Rev:          $Id$
 *
 * Modified:
 *               Copyright (C) 2003 Motorola Corporation.
	...
	case RTC_ALM_SET:	/* Store a time into the alarm */
		{
			...
			bfin_write_RTC_ALARM(((g_alarm_day << DAY_BITS_OFF) |
					      (hrs << HOUR_BITS_OFF) | (min <<
									MIN_BITS_OFF)
					      | (sec << SEC_BITS_OFF)));
			wait_for_complete();
	...
	case RTC_SET_TIME:	/* Set the RTC */
		{
			...
			spin_lock_irq(&rtc_lock);
			bfin_write_RTC_STAT(((day << DAY_BITS_OFF) |
					     (hrs << HOUR_BITS_OFF) | (min <<
								       MIN_BITS_OFF)
					     | (sec << SEC_BITS_OFF)));
			wait_for_complete();
