...
static void
reloc_stack_push(uint32_t value)
{
		       ...
		       struct module *me)
{
	...
	for (i = 0; i < sechdrs[relsec].sh_size / sizeof(*rel); i++) {
		...
		location16 = (uint16_t *)(sechdrs[sechdrs[relsec].sh_info].sh_addr
			+ rel[i].r_offset);
		...
		DEBUGP("location is %x, value is %x type is %d \n", location32, value, ELF32_R_TYPE(rel[i].r_info));
		...
		case R_pcrel24_jump_l:
			...
			value >>= 1;
			DEBUGP("value is %x, before %x-%x after %x-%x\n", value, *location16, *(location16 + 1), (*location16 & 0xff00) | (value>>16 & 0x00ff), value&0xffff);
			*location16 = (*location16 & 0xff00) | (value>>16 & 0x00ff);
